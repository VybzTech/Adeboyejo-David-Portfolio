...
export function FeaturedCaseStudies() {
  ...
  return (
    <section className={cn("py-20 px-6 relative overflow-hidden", theme === "light" ? "bg-white" : "bg-black")}>
      <div className="max-w-7xl mx-auto">
        {/* Metrics Section */}
        <ScrollReveal>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20 p-8 md:p-12 rounded-3xl bg-gradient-to-r from-blue-50/50 to-blue-100/30 dark:from-blue-950/20 dark:to-blue-900/10">
            {metrics.map((metric, idx) => (
              <div key={idx} className="text-center">
                <h3 className="text-5xl md:text-6xl font-bold text-blue-600 mb-2">{metric.value}</h3>
                <p className="text-lg text-slate-700 dark:text-slate-300 font-medium">{metric.label}</p>
              </div>
            ))}
          </div>
        </ScrollReveal>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-16">
          <ScrollReveal direction="left">
            <div>
              ...
              <p className={cn("text-4xl md:text-5xl font-heading font-bold max-w-xl", theme === "light" ? "text-slate-900" : "text-white")}>
