...
import { EnvelopeSimpleIcon, PhoneIcon, LinkedinLogoIcon, PaperPlaneTiltIcon, XCircleIcon } from "@phosphor-icons/react";
...
import { submitContactForm } from "@/app/actions/portfolioActions"; // Importing Server Action
// Telemetry Action Import
...
// import { formSchema, FormValues } from "@/lib/schemas/contact"; 
...
//   const onSubmit = async (data: FormValues) => {
//     setIsSubmitting(true);
//     const loadingToast = toast.loading("Sending your message straight to database...");

//     // Directly invoking Server Action
...
//     if (result.success) {
//       toast.success("Message stored successfully! I'll get back to you soon.", { duration: 5000 });
//       reset();
//     } else {
//       toast.error(result.error || "Failed to submit message.", { duration: 4000 });
//     }
//   };

//   const contactItems = [
//     { icon: <EnvelopeSimpleIcon size={24} weight="fill" />, label: "Email", href: `mailto:${BRAND_INFO.email}` },
//     { icon: <PhoneIcon size={24} weight="fill" />, label: "Phone", href: `tel:${BRAND_INFO.phone}` },
//     { icon: <LinkedinLogoIcon size={24} weight="fill" />, label: "LinkedIn", href: BRAND_INFO.linkedin },
//   ];

//   return (
//     <section id="contact" className={cn("py-32 px-6 relative overflow-hidden", isDark ? "bg-[var(--background)]" : "bg-white")}>
//       <div className="relative z-10 mx-auto w-[80vw] max-w-[82vw]">
//         <motion.div ref={containerRef} whileHover={{ y: -4 }} className={cn("rounded-3xl overflow-hidden border transition-all duration-500", isDark ? "bg-gray-800/80 backdrop-blur-lg border-gray-600/40" : "bg-gradient-to-br from-white via-blue-50/30 to-white border-blue-300/40", "shadow-[0_0_60px_rgba(59,130,246,0.2)]")}>
//           <div className="grid grid-cols-1 lg:grid-cols-2">

//             {/* Left Column Info */}
//             <div className={cn("p-8 md:p-12 lg:p-16 flex flex-col justify-between border-b lg:border-b-0 lg:border-r", isDark ? "bg-[var(--background)] border-gray-600/70" : "bg-white/30 border-blue-200/40")}>
//               <div className="space-y-6">
//                 <div>
//                   <span className="font-medium uppercase text-sm text-primary tracking-[0.8px]">Let's Connect</span>
//                   <h2 className={cn("text-4xl md:text-5xl lg:text-6xl font-heading font-black", isDark ? "text-primary" : "text-black")}>Contact Me</h2>
//                 </div>
//                 <p className={cn("text-base leading-relaxed", isDark ? "text-white" : "text-slate-700")}>
//                   Looking for a Product Engineer who builds scalable, high-performance digital experiences? Let's connect.
//                 </p>
//                 <p className="text-sm font-semibold pt-4 border-t border-slate-500/20">
//                   David Adeboyejo<br />
//                   <span className="text-xs font-normal opacity-80">Product Engineer • Full-Stack Developer</span>
//                 </p>
//               </div>

//               <div className="flex gap-4 pt-8 border-t border-slate-500/20">
//                 {contactItems.map((item) => (
//                   <Link key={item.label} href={item.href} target={item.label === "LinkedIn" ? "_blank" : undefined}>
//                     <button className={cn("flex items-center gap-2 px-4 py-2 rounded-lg hover:cursor-pointer", isDark ? "bg-slate-900 text-white" : "bg-blue-100 text-blue-700")}>
//                       {item.icon}
...
//             {/* Right Column Form */}
//             <div className={cn("p-8 md:p-12 lg:p-16 flex flex-col justify-center", isDark ? "bg-gradient-to-br from-surface to-surface" : "bg-gradient-to-br from-slate-200/50 to-slate-100/35")}>
...
//                 <div className="pt-2">
//                   <PillIconButton type="submit" icon={<PaperPlaneTiltIcon weight="bold" size={20} className="text-white" />} title={isSubmitting ? "Sending..." : "Send Message"} disabled={isSubmitting || Object.keys(errors).length > 0} className="mx-auto" />
//                 </div>
//               </form>
//             </div>
...
export function ContactCTA() {
  ...
  // Telemetry handler for your CTA Link Buttons (Email, Phone, LinkedIn)
  const handleCTAClick = async (label: string) => {
    try {
      await trackInteraction(`Contact CTA - ${label}`, "contact_cta");
    } catch (err) {
      console.error("Telemetry failed quietly:", err);
    }
  };
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (response.ok) {
        // Track the successful submission event in Supabase db
        try {
          await trackInteraction(`Form Submitted by ${data.name}`, "contact_form_submit");
        } catch (telemetryErr) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className={cn(
              ...
              >
                ...
                {/* Description texts */}
                <div className={cn("space-y-4", isDark ? "text-white" : "text-slate-700")}>
                  <p className="text-base leading-relaxed">
                    Looking for a Product Engineer who builds scalable, high-performance digital experiences with strong frontend craftsmanship and full-stack thinking? Let's connect.
                  </p>
                  <p className="text-base leading-relaxed">
              ...
              {/* Dynamic Contact Action Icons */}
              <motion.div
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                viewport={{ once: true }}
                transition={{ delay: 0.3 }}
                className={cn("flex gap-4 pt-8 border-t ", isDark ? "border-slate-800/60" : "border-blue-200/40")}
              >
                {contactItems.map((item, idx) => (
                  <Link
                    key={item.label}
                    ...
                    target={item.label === "LinkedIn" ? "_blank" : undefined}
                    rel={item.label === "LinkedIn" ? "noopener noreferrer" : undefined}
                    onClick={() => handleCTAClick(item.label)} // Injected Telemetry Event
                  >
                    <motion.button
                      initial={{ opacity: 0, y: 10 }}
                      whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className={cn("p-8 md:p-12 lg:p-16",
              ...
              >
                ...
                <div className="relative pt-2">
                  <PillIconButton
                    icon={<PaperPlaneTiltIcon weight="bold" size={20} className="text-white" />}
