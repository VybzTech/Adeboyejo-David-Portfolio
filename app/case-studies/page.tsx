    ...
    <div className="pt-32 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3 mb-16">
            {categories.map(category => {
              const count = category === "All"
                ? PROJECTS.length
                : PROJECTS.filter(p => p.tags.includes(category)).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, index) => (
            <ScrollReveal key={project.id} delay={index * 0.05}>
              <Card className="group h-full flex flex-col overflow-hidden border-white/5 hover:border-primary/20 transition-all duration-500 rounded-2xl">
                <div className="relative h-64 overflow-hidden bg-surface-elevated">
                  <Image
                    src={project.image}
                    alt={project.name}
